...
function validateLogger (logger) {
    ...
    for (const key of ['debug', 'log', 'info', 'warn', 'error']) {
        if (!logger.hasOwnProperty(key) || typeof logger[key] !== 'function') {
...
class Twitch {
    constructor (token, opt = {}) {
        ...
        if (this.logger.hasOwnProperty('setIsSilent') && typeof opt.isSilent === 'boolean') {
            this.logger.setIsSilent(opt.isSilent);
        }
